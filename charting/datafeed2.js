...
Datafeeds.UDFCompatibleDatafeed = function(datafeedURL, updateFrequency) {
	...
	this._barsPulseUpdater = new Datafeeds.DataPulseUpdater(this, updateFrequency || 300 * 1000);	// 10*1000
...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
	...
	var that = this;
	this._send("http://www.hans.com/chart/conf/datafeed_config.php")
    ...
    var configs = {
        ...
        supportedResolutions: ["1", "3", "5", "15", "30", "60", "D", "W", "M"]
    };
    this._setupWithConfiguration(configs);
};
...
Datafeeds.UDFCompatibleDatafeed.prototype._setupWithConfiguration = function(configurationData) {
	console.log("_setupWithConfiguration");
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(symbolName, onSymbolResolvedCallback, onResolveErrorCallback) {
	...
	var that = this;
	var symbolResolveURL;
	var marketcoin;
	var market;
	var coin;

	symbolResolveURL = "http://www.hans.com/chart/conf/symbols.php";

/************
	if(!symbolName.match("CHART:") || symbolName.match("CHART:")){

		marketcoin = symbolName.split("/");
		coin = marketcoin[0].replace("CHART:", "");
		coin.trim();
		market = marketcoin[1].trim();

		symbolResolveURL = "http://www.hans.com/chart/conf/symbol_"+coin.toLowerCase()+"_"+market.toLowerCase()+".php";

	}
***************/


	// if(symbolName == 'CHART:BTC/KRW' || symbolName == 'BTC/KRW'){
	// 	symbolResolveURL = "http://www.hans.com/chart/conf/symbol_btc_krw.php";
	// }else if(symbolName == 'CHART:ETH/KRW' || symbolName == 'ETH/KRW'){
	// 	symbolResolveURL = "http://www.hans.com/chart/conf/symbol_eth_krw.php";
	// }else if(symbolName == 'CHART:BCH/KRW' || symbolName == 'BCH/KRW'){
	// 	symbolResolveURL = "http://www.hans.com/chart/conf/symbol_bch_krw.php";
	// }else if(symbolName == 'CHART:ETC/USDT' || symbolName == 'ETC/USDT'){
	// 	symbolResolveURL = "http://www.hans.com/chart/conf/symbol_etc_usdt.php";
	// }else{
	// 	symbolResolveURL = "http://www.hans.com/chart/conf/symbol_btc_krw.php";
	// }
	...
	if (!this._configuration.supports_group_request) {
		...
		//Send to symbolResolveURL
			this._send(symbolResolveURL, {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(symbolInfo, resolution, rangeStartDate, rangeEndDate, onDataCallback, onErrorCallback) {
	...
	if (rangeStartDate > 0 && (rangeStartDate + '').length > 10) {
		console.log("Got a JS time instead of Unix one");
		throw new Error(['Got a JS time instead of Unix one.', rangeStartDate, rangeEndDate]);
	}

	console.log("rangeStartDate : "+ rangeStartDate + " rangeEndDate : "+ rangeEndDate);
	this._send("http://www.hans.com/chart/conf/dataBind_han.php", {
	...
	.done(function(response) {
		...
		for (var i = 0; i < barsCount; ++i) {
			...
			if (ohlPresent) {
				...
				//console.log("barValue.open : "+barValue.open);
...
Datafeeds.SymbolsStorage = function(datafeed) {
	...
	//this._exchangesList = ['intowin', 'BTC'];
	this._exchangesList = ['BTC/KRW', 'ETH/KRW', 'BCH/KRW'];
...
Datafeeds.SymbolsStorage.prototype._requestFullSymbolsList = function() {
	console.log("_requestFullSymbolsList");
	var that = this;
	...
	try	{
		for (var symbolIndex = 0; symbolIndex < data.symbol.length; ++symbolIndex) {
			...
			var symbolInfo = {
				...
				intraday_multipliers: tableField(data, 'intraday-multipliers', symbolIndex) || ['1', '5', '15', '30', '60', 'D', 'W', 'M'],
...
Datafeeds.SymbolsStorage.prototype._onAnyExchangeResponseReceived = function(exchangeName) {
	console.log("_onAnyExchangeResponseReceived");
...
Datafeeds.DataPulseUpdater.prototype.unsubscribeDataListener = function(listenerGUID) {
	console.log("unsubscribeDataListener");
	this._datafeed._logMessage('Unsubscribing ' + listenerGUID);
	delete this._subscribers[listenerGUID];
};

Datafeeds.DataPulseUpdater.prototype.subscribeDataListener = function(symbolInfo, resolution, newDataCallback, listenerGUID) {
	console.log("subscribeDataListener");
...
Datafeeds.DataPulseUpdater.prototype.periodLengthSeconds = function(resolution, requiredPeriodsCount) {
	console.log("periodLengthSeconds");
	var daysCount = 0;
...
Datafeeds.QuotesPulseUpdater = function(datafeed) {
	console.log("QuotesPulseUpdater");
	this._datafeed = datafeed;
	this._subscribers = {};
